  ...
  $.widget( "mobile.simpledialog", $.mobile.widget, {
		...
		if ( ! event.isPropagationStopped() ) {
			switch (payload.method) {
				...
				case 'button':
					...
					o.butObj[payload.index].trigger(o.touchEvent);
					break;
			}
		} 
	},
	...
	_create: function(){
		...
		if ( o.isInit &&  o.allowReopen ) { 
			...
			pickPage.find( ".ui-header a").bind(o.touchEvent, function(e) {
				e.preventDefault();
				e.stopImmediatePropagation();
				self.close(true);
			});			
	...
	_buildPage: function () {
		...
		screen = $("<div>", {'class':'ui-simpledialog-screen ui-simpledialog-hidden'})
			...
			.bind(o.touchEvent, function(event){
				if ( !o.forceInput ) {
					self.close();
				}
				event.preventDefault();
